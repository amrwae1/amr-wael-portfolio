"use client";

import { motion, useReducedMotion } from "motion/react";
import { motionTokens, standardEase } from "./motion-tokens";

/**
 * The page's reveal gesture, in two weights.
 *
 * It used to be one gesture — one distance, one duration, one curve — applied
 * to all 27 revealed blocks. The intent was rhythm, but an identical entrance
 * on every heading, paragraph and figure stops reading as rhythm and starts
 * reading as a wrapper someone put around everything, which is the single most
 * recognisable motion tell in generated interfaces.
 *
 * So the gesture now fits what it reveals:
 *
 *   · prose (default) — opacity only, at the shorter state duration. Text that
 *     slides into place delays reading for no gain, and a reviewer skimming
 *     this page in two minutes should never wait on a paragraph. Nothing moves;
 *     it is simply there, a beat sooner than before.
 *   · plate — the full rise, for evidence figures and chapter openings. These
 *     are objects arriving rather than words appearing, and they are what the
 *     page is actually asking the reader to look at. Motion here points at the
 *     evidence, which is the one thing on this site worth pointing at.
 *
 * Nothing here carries meaning. Every child is fully present in the DOM and in
 * the accessibility tree before the gesture runs, the reveal fires once, and
 * under `prefers-reduced-motion` all movement is dropped and only a short fade
 * remains. Removing the animation entirely costs no information.
 *
 * `delay` exists for short staggers inside one group. Keep it to a few items:
 * a long stagger makes a reader wait for content already on their screen.
 */
export function Reveal({
  children,
  delay = 0,
  className = "",
  as = "div",
  variant = "prose",
  style,
}: {
  children: React.ReactNode;
  delay?: number;
  className?: string;
  as?: "div" | "li";
  /** "plate" for evidence figures and chapter openings; "prose" for everything else. */
  variant?: "prose" | "plate";
  /** For the rule colour on boundary and objective callouts. */
  style?: React.CSSProperties;
}) {
  const reduce = useReducedMotion();
  const Element = as === "li" ? motion.li : motion.div;

  const rises = variant === "plate" && !reduce;

  return (
    <Element
      /* Hook for the no-JS fallback in the root layout: the reveal's initial
         state is rendered into the server HTML, so without JavaScript to run
         the animation most of the page would stay invisible. */
      data-reveal=""
      className={className}
      style={style}
      initial={rises ? { opacity: 0, y: 16 } : { opacity: 0 }}
      whileInView={rises ? { opacity: 1, y: 0 } : { opacity: 1 }}
      /* Default `amount` ("some") rather than a fraction: a chapter head taller
         than the viewport would never satisfy a fractional threshold. The
         negative bottom margin holds the reveal until the block is properly on
         screen instead of firing at the very edge. */
      viewport={{ once: true, margin: "0px 0px -12% 0px" }}
      transition={{
        duration: reduce
          ? 0.15
          : variant === "plate"
            ? motionTokens.duration.reveal
            : motionTokens.duration.state,
        ease: standardEase,
        delay: reduce ? 0 : delay,
      }}
    >
      {children}
    </Element>
  );
}
